 ...
 * Created by VISISHTA.
 */
public class CustomExtentReportListener implements IReporter{
...
//					}
				...
				buff.append("<td><a href=#m" + m_methodIndex + ">" + qualifiedName(method) + "(description != null && description.length() > 0 ? (" + description + "):)</a> (null == testInstanceName ? : <br>("
										+ testInstanceName + "))</td> <td class=numi style=text-align:left;padding-right:2em>" + firstLine + "<br/></td><td style=text-align:right>" 
										+ formatter.format(calendar.getTime()) + "</td> <td class=numi>"+ timeConversion(end - start) + "</td></tr>");
	...
	protected void generateExceptionReport(Throwable exception, ITestNGMethod method) {
		...
		//writer.print(Utils.shortStackTrace(exception, true)[0]);
		writer.println("</div>");
	}
	...
	private void startSummaryRow(String label) {
		m_row += 1;
		writer.print("<tr(m_row % 2 == 0 ? class=stripe :)><td style=text-align:left;padding-right:2em><a href=#t" + m_testIndex + "><b>" + label + "</b></a>" + "</td>");
	...
	private void tableStart(String cssclass, String id) {
		writer.println("<table cellspacing=0 cellpadding=0 (cssclass != null ? class=" + cssclass + " 	: style=padding-bottom:2em) (id != null ? id=" + id + " : ) >");
		m_row = 0;
	}
